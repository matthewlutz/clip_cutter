...
# TEST MODE: Only analyze first 5 minutes for faster iteration
TEST_MODE = True
...
def analyze_video(video_path: str, query: str, progress_callback=None, cancel_event=None) -> list[dict]:
    ...
    # TEST MODE: Trim to first 5 minutes for faster testing
    if TEST_MODE and duration > TEST_DURATION_SECONDS:
        print(f"TEST MODE: Trimming video from {duration:.0f}s to {TEST_DURATION_SECONDS}s")
        if progress_callback:
            progress_callback(0.02, "TEST MODE: Extracting first 5 minutes...")

        temp_dir = tempfile.mkdtemp(prefix="test_trim_")
        trimmed_path = os.path.join(temp_dir, "trimmed_test.mp4")

        try:
            (
                ffmpeg
                .input(video_path, t=TEST_DURATION_SECONDS)
                .output(trimmed_path, c="copy", avoid_negative_ts="make_zero")
                .overwrite_output()
                .run(cmd=FFMPEG_PATH, quiet=True)
            )
            # Recursively analyze the trimmed video
            result = analyze_video(trimmed_path, query, progress_callback, cancel_event)
            return result
        finally:
            try:
